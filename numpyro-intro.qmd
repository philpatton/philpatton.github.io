...
You might notice that the function lacks a `return` statement, which can look a bit odd. That's because NumPyro *registers* random variables in the function to a *handler*, such as an MCMC sampler. This gives the language and syntax flexibility to exchange handlers and models.
...
As we can see, we get virtually the same estimates as the [PyMC occupancy notebook](occupancy.qmd).
